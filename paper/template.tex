...
The bed of Antarctica is one of the most challenging surface on Earth to map due to the thick layer of ice covering the continent.
By measuring bed elevation, we can estimate the thickness and volume of ice in Antarctica, and by knowing the roughness of the bed topography, we can better quantify how ice flows over the continent.
...
Between 2012 and 2016, the Antarctic Ice Sheet was losing mass at an average rate of 199 ± 26 Gt yr–1 (0.55 ± 0.07 mm yr-1 SLE), with most of the ice loss attributed to the acceleration, retreat and rapid thinning of major West Antarctic Ice Sheet outlet glaciers \citep{MeredithPolarregions2019}.
The roughness of the bed plays an important role in the flow of these outlet glaciers.
...
The bed is therefore an essential component in ice sheet models that quantify projections of future sea level rise.
This fact is a key justification for the work put in by researchers over the past two decades to compile bed elevation models of Antarctica, notably with the BEDMAP1 \citep{LytheBEDMAPnewice2001} and BEDMAP2 \citep{FretwellBedmap2improvedice2013} products.

Two main techniques exist for estimating the subglacial terrain of Antarctica.
The main method is via the use of ice-penetrating radar - a method that images the ice-bed interface along survey lines \citep[e.g.][]{ShiMultichannelCoherentRadar2010}.
This method does come with uncertainties due to different interpretations of radargrams, owing to uncertainties due to changing firn density, and potential errors in the semi-automatic bed picks.
Still, this is the most direct method, though it is geographically limited and involves manual, time consuming work.
...
The shift towards higher spatial resolution Digital Elevation Model (DEM) becomes more urgent as ice sheet models move to using sub-kilometer grids in order to quantify glacier ice flow dynamics more accurately.
A finer grid is especially important at the ice sheet's grounding zone where adaptive mesh refinement schemes focus on \citep[e.g.][]{CornfordAdaptivemeshrefinement2016}, and wherein attention to the bed roughness component is imperative for proper modelling of fast flowing outlet glaciers \citep{DurandImpactbedrockdescription2011,NiasContrastingmodelledsensitivity2016}.
The challenge of producing a high resolution DEM while preserving a realistic representation of the bed terrain's roughness is a problem we attempt to address.

Here, we present a deep neural network method that belongs to the inverse modelling category, but one that is trained on direct ice-penetrating radar observations over Antarctica.
...
They are trained using backpropagation, a procedure whereby the weights or parameters of the neurons' connections are adjusted, so as to minimize the error between the groundtruth and output of the neural network \citep{RumelhartLearningrepresentationsbackpropagating1986}.
...
Convolutional neural networks differ from standard artificial neural networks in that they use kernels or filters in place of regular neurons \citep[see][for a review]{LeCunDeeplearning2015}.
They are especially prevalent in the computer vision community, having existed since the 1980s \citep{FukushimaNeocognitronnewalgorithm1982,LeCunBackpropagationAppliedHandwritten1989} and are commonly used in visual pattern recognition tasks \citep[e.g.][]{LecunGradientbasedlearningapplied1998,KrizhevskyImageNetClassificationDeep2012}.
...
One promising approach is to use use deep neural networks \citep{LeCunDeeplearning2015} to learn an end-to-end mapping between the low and high resolution images, a method developed originally in the Super-Resolution Convolutional Neural Network paper \citep[SRCNN,][]{DongImageSuperResolutionUsing2014}.

Since SRCNN, multiple developments have been made to further improve the perceptual quality of super resolution neural networks \cite[see][for a review]{YangDeepLearningSingle2018}.
...
By removing unnecessary model components and adding residual \citep{HeDeepResidualLearning2015} connections, the Enhanced Deep Super-Resolution network \citep[EDSR,][]{LimEnhancedDeepResidual2017} features a deeper neural network model that has better performance than older models.
For our DeepBedMap model in this paper, we choose to adapt an Enhanced Super-Resolution Generative Adversarial Network \citep[ESRGAN,][]{WangESRGANEnhancedSuperResolution2018} that brings together the ideas mentioned above.
This approach produces state of the art perceptual quality and won the 2018 Perceptual Image Restoration and Manipulation (PIRM) Challenge on Super-Resolution (Third Region) \citep{Blau2018PIRMChallenge2018}.
...
Zooming in closer, we highlight a few qualitative observations of DeepBedMap\_DEM's bed topography at Pine Island Glacier (see Figure \ref{fig:3}) and other parts of Antarctica (see Figure \ref{fig:4}).
DeepBedMap\_DEM shows a terrain with realistic topographical features, having fine-scale bumps and troughs that makes it rougher than that of BEDMAP2 \citep{FretwellBedmap2improvedice2013} and Bedmachine Antarctica \citep{MorlighemMEaSUREsBedMachineAntarctica2019} while still preserving the general topography of the area (see Figure \ref{fig:3}).
Over steep topographical areas such as the Transantarctic Mountains and Gamburtsev  Subglacial Mountains (see Figure \ref{fig:4a}, \ref{fig:4h}, \ref{fig:4i}), DeepBedMap produced speckle texture patterns.
Along fast flowing ice streams and glaciers (see Figure \ref{fig:4b}, \ref{fig:4c}, \ref{fig:4d}, \ref{fig:4e}, \ref{fig:4f}, \ref{fig:4g}, \ref{fig:4h}), we can see ridges aligned parallel to the sides of the valley.
Over relatively flat regions in both West and East Antarctica (e.g. Figure \ref{fig:4g}), there are some wave-like patterns occasionally represented in the terrain.
...
The calculated 2D roughness for DeepBedMap (Figure \ref{fig:5b}) and the Groundtruth (Figure \ref{fig:5c}) lie in a similar range from 0 m to 400 m in standard deviation, with small clusters of areas with high roughness values.
In contrast, the roughness of BEDMAP2 (Figure \ref{fig:5d}) is mostly in the 0 m to 100 m range, and those high roughness cluster areas tend to have a larger area.
...
Taking a 1D transect over these 250 m resolution grids, we illustrate the differences in bed topography and roughness from the coast towards the inland area of Thwaites Glacier (see Figure \ref{fig:6}).
DeepBedMap\_DEM manages to capture much of the rough topography found in the Operation IceBridge groundtruth data, especially near the coast (see Figure \ref{fig:6}, from -1550 to -1500 on x-scale) where the terrain tends to be rougher.
Along the retrograde slope (see Figure \ref{fig:6}, from -1500 to -1450 on x-scale), several of the fine-scale bumps and troughs in DeepBedMap\_DEM can be seen to correlate well in position with the groundtruth.
In contrast, the cubic interpolated BEDMAP2 product lacks such fine-scale bumps and troughs, appearing as a relatively smooth terrain over much of the transect.
Looking at the roughness statistic, both the DeepBedMap\_DEM and Operation IceBridge groundtruth grids show a tendency towards higher values compared to that of BEDMAP2, the former two having a mean standard deviation of about 40m compared to the latter with a mean of about 10 m.
Indeed, the BEDMAP2 product has a relatively smooth surface that rarely exceeds a standard deviation value of 20 m along this transect.

\begin{figure}[htbp]
  \includegraphics[width=0.9\columnwidth]{figures/fig6_elevation_roughness_transect.png}
  \caption{
    Comparing bed elevation and surface roughness (standard deviation of elevation values) of each interpolated grid product (250 m resolution) over a transect (see Figure \ref{fig:5} for location of transect line).
...
We are not sure why speckle texture patterns are found over steep mountains, but it is clear that the severe lack of high resolution training datasets leads our model to perform worse over these high gradient areas.
The ridges found along fast flowing ice streams and glaciers are likely to be streaklines typical along the bed of such regions, formed from the scouring effect of ice.
Finally, we interpret the wave-like patterns to be megadune structures \citep{ScambosSnowMegadune2014} that our DeepBedMap model has likely picked up from using the high resolution ice surface elevation model \citep{HowatReferenceElevationModel2019}.
...
Besides collecting more radio-echo sounding datasets to sample these regions more densely, swath reprocessing of old datasets that have that capability may prove useful to adding to the training set \citep{HolschuhSwathtopographyfuture2019}.
...
Our DeepBedMap convolutional neural network method presents a data-driven approach to resolve the bed topography of Antarctica.
...
Our method emphasizes the value of densely spaced bed elevation datasets sampled over a more diverse portion of Antarctica.
There is a need for more high resolution surveys (<250 m line spacing), preferably swath datasets, that capture a more representative picture of Antarctica's varied bed topography.
